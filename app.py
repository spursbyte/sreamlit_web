...
if st.button("Visualization created by Author "):
    ...
    st.markdown(" **Takeaway:** Gento is *most massive & most gigantic* with an average body mass 5.08 Kg & average flipper length 217 mm.")
...
if st.sidebar.checkbox('Graphics'):
    ...
    if st.sidebar.checkbox('Boxplot'):
            ...
            st.info("If error, please adjust column name on side panel.")
            column_box_plot_X = st.sidebar.selectbox("X (Choose a column). Try Selecting island:",
                                                     df.columns.insert(0, None))
            column_box_plot_Y = st.sidebar.selectbox("Y (Choose a column - only numerical). Try Selecting Body Mass",
                                                     df.columns)
            hue_box_opt = st.sidebar.selectbox("Optional categorical variables (boxplot hue)",
                                               df.columns.insert(0, None))
...
st.sidebar.info("Self Exploratory Visualization on palmerpenguins - Brought To you By [sid star](https://github.com/spursbyte)  ")
